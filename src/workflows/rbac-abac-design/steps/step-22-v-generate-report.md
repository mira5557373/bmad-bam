...
Generate a comprehensive validation report summarizing findings from the RBAC/ABAC architecture validation steps and determining the workflow completion status.
...
|----------|--------|-------|
| Permission Model | | Taxonomy, resource mappings, boundaries |
| Role Hierarchy | | Inheritance, separation of duties |
| Attribute Policies | | PDP architecture, conditions |
| Tenant Scoping | | Isolation, audit requirements |
| Cross-Cutting | | Consistency, compliance |

### 2. Assign Severity to Findings

| Severity | Description | Action Required |
|----------|-------------|-----------------|
| CRITICAL | Blocks security validation | Must fix before proceeding |
| WARNING | Non-critical security gap | Should address before production |
| INFO | Improvement opportunity | Consider for future iterations |

**Critical Failure Examples:**
- Missing permission taxonomy
- No role inheritance defined
- Undefined tenant isolation
- Cross-tenant access not controlled

### 3. Determine Completion Status

| Status | Criteria |
|--------|----------|
| **COMPLETE** | All 4 components defined, security baseline met, tenant isolation verified |
| **CONDITIONAL** | Minor gaps with mitigation plan |
| **NEEDS REVISION** | Missing permission model, role hierarchy, or tenant scoping |
...
- Context: "Review RBAC/ABAC validation report and recommend next steps"
...
- RBAC/ABAC Architecture Validation Report
- Component-level findings summary
- Gate decision documentation
- **Output to:** `{output_folder}/planning-artifacts/rbac-abac-validation-report.md`
...
Validation mode complete for rbac-abac-design workflow.
