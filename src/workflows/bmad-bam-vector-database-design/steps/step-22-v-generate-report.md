...
Generate a comprehensive validation report summarizing findings from the vector database design validation steps and determining the workflow completion status.

---

## Prerequisites

- Step 20 completed: Vector database design artifact loaded successfully
...
|----------|--------|-------|
| Requirements | | Embedding dimensions, scale, latency |
| Index Strategy | | Algorithm selection, parameters |
| Tenant Isolation | | Pattern, namespace, access control |
| Query Optimization | | Caching, filtering, hybrid search |
| Scaling | | Sharding, replicas, auto-scaling |
| Backup/Recovery | | RPO/RTO, procedures |
...
- Missing tenant isolation mechanism
...
| Status | Criteria |
|--------|----------|
| **COMPLETE** | All components defined, tenant isolation verified, security complete |
| **CONDITIONAL** | Minor gaps with documented mitigation plan |
| **NEEDS REVISION** | Missing tenant isolation, security controls, or scaling strategy |
...
- Context: "Review vector database validation report and recommend next steps"
...
- Vector Database Design Validation Report
- Component-level findings summary
- Gate decision documentation
- **Output to:** `{output_folder}/planning-artifacts/vector-database-validation-report.md`
...
- **COMPLETE:** Workflow finished. Proceed to downstream workflows (embedding-strategy-design).
...
Validation mode complete for vector-database-design workflow.
