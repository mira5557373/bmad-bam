...
Load and review existing self-service upgrade documents to identify sections requiring modification.

---

## Prerequisites

- Existing self-service upgrade documents to modify
...
|-----------|--------|-------------------|
| Upgrade Paths | Yes/No | {paths defined} |
| Flow Steps | Yes/No | {steps count} |
| UI Components | Yes/No | {components defined} |
| Payment Provider | Yes/No | {provider selected} |
| Proration Logic | Yes/No | {scenarios covered} |
| Provisioning Pipeline | Yes/No | {steps, timing} |
| Rollback Procedure | Yes/No | {rollback defined} |
| Operational Runbook | Yes/No | {procedures count} |
...
- **P (Party Mode)**: Bring product perspectives on proposed changes
...
- Context: "Review proposed modifications to self-service upgrade design"
- Process product and billing perspectives on change impact
...
- Summary of current self-service upgrade state
