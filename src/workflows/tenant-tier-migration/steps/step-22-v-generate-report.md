...
Generate a comprehensive validation report summarizing findings from the tier migration validation steps and determining the workflow completion status.

---

## Prerequisites

- Step 20 completed: Tier migration artifact loaded successfully
...
|----------|--------|-------|
| Tier Definitions | | All tiers, features, limits |
| Migration Paths | | Upgrade/downgrade paths |
| Upgrade Strategy | | Enablement sequence |
| Downgrade Strategy | | Data retention, degradation |
| Feature Transitions | | Flags, entitlements |
| Runbook | | Pre-flight, execution, monitoring |
| Rollback Procedure | | Triggers, window, emergency |
...
|----------|-------------|-----------------|
| CRITICAL | Blocks safe migration | Must fix before proceeding |
...
- No data retention policy for downgrades
- Missing pre-flight checks
- No graceful degradation for premium features

### 3. Determine Completion Status

| Status | Criteria |
|--------|----------|
| **COMPLETE** | All components defined, critical checks pass, runbook tested |
...
- Context: "Review tier migration validation report and recommend next steps"
...
- Tier Migration Validation Report
- Component-level findings summary
- Gate decision documentation
- **Output to:** `{output_folder}/planning-artifacts/operations/tier-migration-validation-report.md`
...
- **COMPLETE:** Workflow finished. Tier migration plan is ready for implementation.
...
Validation mode complete for tenant-tier-migration workflow.
