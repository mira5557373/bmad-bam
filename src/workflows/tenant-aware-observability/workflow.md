...
**Use when:**
- Designing observability strategy for a multi-tenant platform
- No tenant-aware observability spec exists
- Adding cost attribution and usage tracking per tenant
- Preparing for production readiness (QG-P1 requires observability)

**Do NOT use when:**
- Observability spec already exists (use Edit mode)
- Designing usage metering specifically (use `usage-metering-design` workflow)
- Only configuring dashboards (use operational procedures)
- Foundation is not complete (complete `create-master-architecture` first)

**Prerequisites:**
- `master-architecture.md` with tenant model defined
- Understanding of observability stack (Prometheus, Grafana, Langfuse, etc.)
- List of SLOs and SLIs for tenant-facing services
- Cost allocation requirements for multi-tenant billing
