...
#   - Filename `bam-platform-project-context.md` keeps BAM's content separate from
...
# Target directory + file — BMM-aligned: {output_folder}/bam-platform-project-context.md.
# Survives BMAD installer's fs.remove(targetPath) because {output_folder} is outside
# _bmad/<module-code>/.
TARGET_DIR="$OUTPUT_FOLDER"
TARGET_FILE="$TARGET_DIR/bam-platform-project-context.md"
