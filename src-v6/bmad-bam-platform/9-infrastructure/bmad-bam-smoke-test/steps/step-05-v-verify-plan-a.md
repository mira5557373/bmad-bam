...
2. The sentinel `project-context.md` exists at a path the glob would match when the LLM runs. BAM uses the BMM-aligned canonical location: `{output_folder}/bbp/project-context.md` (default `_bmad-output/bbp/project-context.md`), outside `_bmad/<module-code>/` so BMAD's install-time wipe doesn't touch it. The universal-glob `**/project-context.md` matches it because the filename ends in `project-context.md` and `**` matches arbitrary depth.
...
# Part 2: sentinel project-context.md exists at glob-matching path (BMM-aligned).
...
if [ ! -f "$SENTINEL_FILE" ]; then
    echo "PLAN_A_FAIL: sentinel project-context.md not present at $SENTINEL_FILE"
    exit 1
fi
