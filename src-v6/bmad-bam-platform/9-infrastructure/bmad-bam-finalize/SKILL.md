...
description: "One-shot activation finalizer for the BAM v6 platform module. Generates {output_folder}/bbp/project-context.md (the universal-glob sentinel). Run once after `bmad install bmad-bam-platform`."
---
...
write — `{output_folder}/bbp/project-context.md` — to opt the host project into the
...
- Whenever `{project-root}/{output_folder}/bbp/project-context.md` is missing.
- Whenever the BAM platform module is upgraded (re-running refreshes the sentinel
  and the install log; safe and idempotent).

## Output

- Writes (or refreshes) `{project-root}/{output_folder}/bbp/project-context.md`.
