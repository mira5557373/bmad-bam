...
outputs: ["{output_folder}/bbp/project-context.md", _bmad/bam/install-logs/platform-install.log, _bmad/_memory/atlas/architecture-decisions/INDEX.md]
---
...
Generate (or refresh) `{project-root}/{output_folder}/bbp/project-context.md` — the
...
2. **Locate `scripts/post-install.sh`.** It ships inside this finalize skill's
   ...
   ```bash
   SCRIPT="$PROJECT_ROOT/_bmad/bbp/bmad-bam-finalize/scripts/post-install.sh"
   if [ ! -x "$SCRIPT" ]; then
       # Source-tree fallback for development checkouts:
       SCRIPT="$PROJECT_ROOT/src-v6/bmad-bam-platform/9-infrastructure/bmad-bam-finalize/scripts/post-install.sh"
   fi
   if [ ! -x "$SCRIPT" ]; then
       echo "ERROR: post-install.sh not found or not executable." >&2
       echo "       Expected: _bmad/bbp/bmad-bam-finalize/scripts/post-install.sh (installed)" >&2
       echo "       Or:       src-v6/bmad-bam-platform/9-infrastructure/bmad-bam-finalize/scripts/post-install.sh (dev checkout)" >&2
       exit 73
   fi
   ```
...
4. **Seed Atlas's sidecar memory directories (idempotent).** Downstream BAM
   ...
   (`_bmad/bbp/`), so it survives re-installs. Existing ADRs are
...
lives at `{output_folder}/bbp/project-context.md`; resolve
...
test -f "$SENTINEL_FILE" \
    || { echo "FAIL: project-context.md not created at $SENTINEL_FILE" >&2; exit 1; }
