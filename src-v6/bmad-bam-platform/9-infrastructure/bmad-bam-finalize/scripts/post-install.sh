...
<!-- Do not edit by hand; regenerated on every \`/bmad-bam-finalize\` invocation -->
...
- **bmad-bam-design-tenancy-model** — first BAM workflow (P2.1). Produces
  ...
  Invoke via \`/bmad-bam-design-tenancy-model\` (Claude Code/Cursor).

## Module status

Module installed via Path B (manual finalize) per spec §7.6 / P2.1 Task 0.
Re-invoke \`/bmad-bam-finalize\` after any \`bmad install bmad-bam-platform\`
