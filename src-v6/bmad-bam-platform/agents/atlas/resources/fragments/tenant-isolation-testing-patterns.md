...
- `design-tenancy-model` skill (`src-v6/bmad-bam-platform/skills/bmad-bam-design-tenancy-model/`) — primary consumer. `step-05-c-write-design` references this fragment for the §5 isolation-tests catalogue; `step-07-v-verify-completeness` checks that every test type listed in §5 is realized in code.
